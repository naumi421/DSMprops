...
# saveRDS(frags_chkey,paste0(projfldr,"frags_chkey.rds"))
frags_chkey <- readRDS(paste0(projfldr,"frags_chkey.rds"))
## chkeys <- unique(horiz_comps$chkey)
## chfrags <- chfrags[chfrags$chkey %in% chkeys,]
horiz_comps <- left_join(horiz_comps,frags_chkey, by="chkey")
horiz_comps$fragvol_r <- ifelse(is.na(horiz_comps$fragvol_r), 0, horiz_comps$fragvol_r) # NAs set to zero as these are not recorded in chfrags table
horiz_comps <- subset(horiz_comps, horiz_comps$fragvol_r < 101) ## Removing records with >100% recordings as these are obviously erroneous (n = 37 components)
