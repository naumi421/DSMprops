...
predfolder <- "/mnt/disks/sped/solus100preds/v2tst_gnat_pts/Clay_gRPI_250k"
...
prop <- "claytotal_r" ## Dependent variable: UPDATE EVERY TIME
...
for(d in depths){
  ...
  # pts.gNAT.d$prop <- lapply(pts.gNAT.d.ids, function(x) sample(as.numeric(pts.gNAT.d[x,163:165]),size=1,replace = T,prob = as.numeric(pts.gNAT.d[x,166:168])))
  # pts.gNAT.d$prop <- as.numeric(pts.gNAT.d$prop)
  pts.gNAT.d$prop <- as.numeric(pts.gNAT.d[,paste0(prop,"_mean_",datastretchlab,"_",d,"_cm_gNATSGO")])/datastretch
  pts.gNAT.d[,163:165] <- NULL
  ...
  rasterOptions(maxmemory = 6e+09,chunksize = 7e+08)# maxmemory = 6e+09,chunksize = 6e+08 40-50% cpu, 530GB
  beginCluster(cpus,type='SOCK')
  Sys.time()
