...
predfolder <- "/mnt/disks/sped/solus100preds/v2tst_gnat_pts/pH_gRPI_250k"
...
prop <- "ph1to1h2o_r" ## Dependent variable: UPDATE EVERY TIME
...
## SCD prep for RF
scd.pts.ext.hor$prop <- scd.pts.ext.hor$ph_h2o ## UPDATE everytime!
scdprop <- "ph_h2o"
scd.pts.ext.hor$tid <- "scd"
hist(scd.pts.ext.hor$prop)
summary(scd.pts.ext.hor$prop)
scd.pts.ext.hor <- subset(scd.pts.ext.hor, scd.pts.ext.hor$prop >= 1) # Eliminating low values not possible for pH (typos?).
