...
predfolder <- "/mnt/covs/solus_preds/v2tst_gnat_pts/Gypsum_gRPI_250k"
repofolder <- "/mnt/disks/sped/repos/DSMprops"
covfolder <- "/mnt/covs/covs100m"
propcovfldr <- "/mnt/covs/gNATSGO_CONUS_tif/covs100m_by_prop"
...
prop <- "gypsum_r" ## Dependent variable: UPDATE EVERY TIME
...
### Gypsum specific processing: dealing with gyp20 vs c_gyp2
gyp_trn_st <- scd.hor[(!is.na(scd.hor$gypl20))&(!is.na(scd.hor$c_gypl2)),]
gyp_translate_lm <- lm(gypl20 ~ c_gypl2, data = gyp_trn_st) ## gyp20 = 0.9642 * gypl2 + 0.6360, Rsq = 0.989
scd.pts.ext.hor$gypl20_p <- predict(gyp_translate_lm, newdata = scd.pts.ext.hor)
...
scd.pts.ext.hor[is.na(scd.pts.ext.hor$gypl20_p),"gypl20_p"] <- 0
scd.pts.ext.hor <- scd.pts.ext.hor[scd.pts.ext.hor$gypl20_p <120,] # knocking off a 8 high value transcription errors
scd.pts.ext.hor$gypl20_p <- ifelse(scd.pts.ext.hor$gypl20_p > 100, 100, scd.pts.ext.hor$gypl20_p)
summary(scd.pts.ext.hor$gypl20_p)

## SCD prep for RF
scd.pts.ext.hor$prop <- scd.pts.ext.hor$gypl20_p ## UPDATE everytime!
scdprop <- "gypl20"
