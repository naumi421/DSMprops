...
predfolder <- "/mnt/disks/sped/solus100preds/v2tst_gnat_pts/CEC7_gRPI_250k"
...
prop <- "cec7_r" ## Dependent variable: UPDATE EVERY TIME
...
ldm <- lapply(c("NCSS_Layer","NCSS_Site_Location","CEC_and_Bases","NCSS_Pedon_Taxonomy"), function(x) dbReadTable(con , x))
names(ldm) <- c("NCSS_Layer","NCSS_Site_Location","CEC_and_Bases","NCSS_Pedon_Taxonomy")
dbDisconnect(con)

## Now wrangle tables
scd.hor <- inner_join(ldm$NCSS_Layer,ldm$CEC_and_Bases[!duplicated(ldm$CEC_and_Bases$labsampnum),],by="labsampnum")
...
## SCD prep for RF
scd.pts.ext.hor$prop <- scd.pts.ext.hor$cec_nh4 ## UPDATE everytime!
scdprop <- "cec_nh4"
