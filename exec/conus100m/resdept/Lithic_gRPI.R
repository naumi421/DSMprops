...
predfolder <- "/home/tnaum/data/Hyb100m/RestrDepth_gRPI"
...
## Join extracted pts to restriction data
pts.lithic <- readRDS(paste0(predfolder,"/nasis_anylithic.rds"))
pts.ext.hor <- left_join(pts.ext, pts.lithic[,c("peiid","anylithicdpt")], by="peiid")

## Prep nasis training data for Random Forest
pts.ext.hor$prop <- pts.ext.hor$anylithicdpt  ## UPDATE EVERY TIME
prop <- "anylithicdpt" ## Dependent variable
...
## Now wrangle tables
scd.hor <- readRDS(paste0(predfolder,"/scd_anylithic.rds"))
...
scd.pts.ext.hor <- inner_join(scd.pts.ext, scd.hor, by="site_key")
scd.pts.ext.hor <- scd.pts.ext.hor[!duplicated(scd.pts.ext.hor$locid),]

## SCD prep for RF
scd.pts.ext.hor$prop <- scd.pts.ext.hor$Depth  ## UPDATE everytime!
scdprop <- "Depth"
scd.pts.ext.hor$tid <- "scd"
hist(scd.pts.ext.hor$prop)
summary(scd.pts.ext.hor$prop)
scd.pts.ext.hor$prop <- ifelse(is.na(scd.pts.ext.hor$prop),201, scd.pts.ext.hor$prop) ## Sites not measured assumed to not have restriction

## Now bring in outcrops points created from SSURGO
# outcr_pts <- readRDS(paste0(predfolder,"/outcr_polypts.rds"))
# outcr_pts <- spTransform(outcr_pts, cov.proj)
# outcr_pts.ext <- DSMprops::parPTextr(outcr_pts, cov.grids, os = "linux", nthreads=30)
# saveRDS(outcr_pts.ext,paste0(predfolder,"/outcr_pts_ext.rds"))
outcr_pts.ext <- readRDS(paste0(predfolder,"/outcr_pts_ext.rds"))
outcr_pts.ext$prop <- 0 ## These are bedrock outcrops with a restriction at depth 0
## Clean up fields
outcr_pts.ext <- outcr_pts.ext %>%
  rename(x_std = x, y_std = y)
outcr_pts.ext$tid <- "psuedo"
outcr_pts.ext$mtchtype <- "psuedo"
outcr_pts.ext$geo_wt <- 2
outcr_pts.ext$locid <- paste0(outcr_pts.ext$y_std,outcr_pts.ext$x_std)
outcr_pts.ext$peiid <- outcr_pts.ext$locid
...
d <- "all"
  ...
  ## Outcrop points
  outcr.pts.d <- outcr_pts.ext[,c(ptspred.list)]
  outcr.pts.d <- na.omit(outcr.pts.d)
  pts.extcc <- rbind(pts.extcc,scd.pts.d,outcr.pts.d)
  ...
  rf.qrf <- ranger(formulaStringRF_RFE, data=pts.pcv@data, num.trees = trn.params$ntrees, quantreg = T, num.threads = 30,
  ...
  pts.pcv@data$trainpreds <- predict(rf.qrf, data=pts.pcv@data, num.threads = 30)$predictions
  attach(pts.pcv@data)
