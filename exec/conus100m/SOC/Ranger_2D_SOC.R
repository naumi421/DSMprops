...
predfolder <- "/push/HYBconus100m/SOC"
repofolder <- "/push/repos/DSMprops/exec/conus100m/SOC"
...
pts.ext.hor$soc_r <- 0.58 * pts.ext.hor$om_r # Covert to org carbon
pts.ext.hor$prop <- pts.ext.hor$soc_r ## UPDATE EVERY TIME
prop <- "soc_r" ## Dependent variable
...
ldm <- lapply(c("NCSS_Layer","NCSS_Site_Location","Carbon_and_Extractions","NCSS_Pedon_Taxonomy"), function(x) dbReadTable(con , x))
names(ldm) <- c("NCSS_Layer","NCSS_Site_Location","Carbon_and_Extractions","NCSS_Pedon_Taxonomy")
dbDisconnect(con)

## Now wrangle tables
scd.hor <- inner_join(ldm$NCSS_Layer,ldm$Carbon_and_Extractions[!duplicated(ldm$Carbon_and_Extractions$labsampnum),],by="labsampnum")
