...
predfolder <- "/mnt/covs/solus_preds/v2tst_gnat_pts/RestrDepth_gRPI_250k"
...
prop <- "anylithicdpt" ## Dependent variable: UPDATE EVERY TIME
...
## Now wrangle tables
scd.hor <- readRDS(paste0(predfolder,"/scd_anylithic_v2.rds"))
...
# rasterOptions(maxmemory = 2e+10)
# scd.pts.ext <- DSMprops::parPTextr(scd.pts, cov.grids, os = "windows", nthreads=cpus)
...
##### Model training and prediction block (adapted from depthwise predictions for other properties...)
d <- "all"
...
pts.extcc <- rbind(pts.extcc,scd.pts.d,pts.gNAT.d,outcr.pts.d)
