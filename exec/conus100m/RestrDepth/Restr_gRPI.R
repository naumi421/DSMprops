...
predfolder <- "/home/tnaum/data/Hyb100m/RestrDepth"
...
pts.restr <- readRDS(paste0(predfolder,"/nasis_restrs.rds"))
pts.ext.hor <- left_join(pts.ext, pts.restr[,c("peiid","resdept_r")], by="peiid")

## Prep nasis training data for Random Forest
pts.ext.hor$prop <- pts.ext.hor$resdept_r  ## UPDATE EVERY TIME
prop <- "resdept_r" ## Dependent variable
...
## Now wrangle tables
scd.hor <- readRDS(paste0(predfolder,"/scd_restrs.rds"))
...
## SCD prep for RF
scd.pts.ext.hor$prop <- scd.pts.ext.hor$resdept_r  ## UPDATE everytime!
scdprop <- "resdept_r"
