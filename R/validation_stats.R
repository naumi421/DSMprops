...
#' @export
#'
valmetrics <- function(xlst, trans, quants, prop, depth){
  ...
  valfn <- function(v){
    ...
    varrange <- as.numeric(quantile(pts.extpcv$prop, probs=c(quants[2]), na.rm=T)-quantile(pts.extpcv$prop, probs=c(quants[1]),na.rm=T))
